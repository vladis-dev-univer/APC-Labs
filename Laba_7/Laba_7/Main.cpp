...
void Server(char* path)
{
	...
	handler = CreateFile(
		(LPSTR)name.c_str(),                            // Имя открываемого файла.
	...
	CreateProcess(
		(LPSTR)path,                                // Имя исполняемого модуля.
	...
	while (true)
	{
		...
		cin.clear();
		getline(cin, message);
		if (message == "Exit") {                                  // Условие выхода из программы.
...
void Client()
{
	...
	hMyPipe = CreateFile((LPSTR)name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); //асинхронный режим
